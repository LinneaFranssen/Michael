...
double HawkHawkWin = 0.5*(V-C) ; //here 1 H wins and one loses; no average as above scenario //05/07/17: This is negative though!?
...
////// HOUSEKEEPING CREATE VARIABLES AND RANDOM NUMBER AND GET ARGUMENTS
    ...
    int MaxWasps = 1; //4 // maximum no of wasps interactions per timestep. 05/07/17: NOTE THAT THIS VARIABLE IS MISLEADING: if you run it with 1, some individua interact twice when setting time step to 1 //must check that this MaxWasp business does not have a bug!!!
...
*/
        ...
        for(int wwk=0;wwk<Wasps[now].size();wwk++)
        {
            ...
            int wgd = Wasps[now][wwk]->getCell()->getGpos();                      // wwk is position of wasp in vector
