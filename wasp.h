...
public: //these are things one can "tell" wasp; here one can decide which things out of 'private' one can ask a wasp object
	...
	void setChiIncrement(double val){m_chiIncrement = val;} //what this does is set a value (m_kProd) but this would normally go into cpp file if it was longer
...
//	void setKProd(double val) {m_KProd = val;}
	...
	void incrementFitness(double val) {m_fitness = m_fitness + val;} //if in ma                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             in interaction occurs with payoff 0.5, would set val=0.5
	
	double getPlayed(){return m_played;}  //next 3 lines is what we call in main if agent changes playing status by playing a game
	...
	double getProbDoveGivenHawk() {return m_probDoveGivenHawk;} //call this to know strat of parent upon their death if they produce offspring and we use sync. prolif. //this is really P(play Dove) as we do not work with cond P // ask this and would get 0 or 1 unless there is a mutation allowed ton change from 1 or 0
...
//	int  getOpponentsLastMove() {return m_opponentsLastMove;} 
	...
	void setprobWaspDeathAferLifespan(double val){m_probWaspDeathAferLifespan = val;} //this is for the prob that it lives above 100/lifespann (if it was in constructor, it would set automatically but this way we have to set it up for each wasp)
