...
int cell::getWaspIndex(wasp *Wasp) {
    int temp = 0;
    for (int i = 0; i < m_Wasps.size(); i++) {
        //loop through every m_Wasp vector entry until reached size of m_Wasps.
        if (m_Wasps[i] == Wasp) {
            temp = i;
        }
    }
